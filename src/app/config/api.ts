    ...
    if (error.response) {
      ...
      switch (status) {
        case 401: {
          ...
          const isAuthCall = requestUrl.includes('/auth/login') || requestUrl.includes('/auth/registration') || requestUrl.includes('/auth/password/change') || requestUrl.includes('/users/update_profile') || requestUrl.includes('materials/') || requestUrl.includes('guest-estimate/');
          if (!isAuthCall) {
